...
class Agent(object):
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            sy_logits_na = policy_parameters
            # YOUR_CODE_HERE
            # define distribution from policy_parameters
            dist = tf.distributions.Categorical(logits=sy_logits_na)
    ...
    def build_computation_graph(self):
        ...
        #========================================================================================#
        loss = -1.0 * tf.reduce_mean(self.sy_logprob_n*self.sy_adv_n)
        ...
        if self.nn_baseline:
            ...
            baseline_loss = tf.reduce_mean( tf.square( self.baseline_prediction - self.sy_target_n ) )
    ...
    def sample_trajectory(self, env, animate_this_episode):
        ...
        while True:
            ...
            ac = self.sess.run(self.sy_sampled_ac, {self.policy_parameters:policy})
            ac = ac[0]
            acs.append(ac)
    ...
    def sum_of_rewards(self, re_n):
        ...
        if self.reward_to_go:
            ...
            for r in re_n:
                total = r.shape[0]
                discount = np.stack( [np.exp(np.linspace(0.0, (total-1)*np.log(self.gamma), total))] * total)
                discount /= np.reshape(np.exp(np.linspace(0.0, (total-1)*np.log(self.gamma), total)), [total, 1])
                lower_ind = np.tril_indices(total)
                discount[lower_ind]=0.0
                np.fill_diagonal(discount, 1.0)
                q = discount @ r
                q_n = np.concatenate((q_n, q))
            ...
            for r in re_n:
                total = r.shape[0]
                discount = np.exp( np.linspace(0, (total-1)* np.log(self.gamma), total) )
                q_n += [ np.sum(discount * r) ] * total
            #raise NotImplementedError
            q_n = np.array(q_n)
        return q_n
